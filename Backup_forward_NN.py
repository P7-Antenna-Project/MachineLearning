...
def db_to_linear(data, inverse: bool):
    if inverse:
        data_out = 10*np.log10(data)
    else:
        data_out = 10**(data/10)
    return data_out
...
def normalize_data(data_input, mean, std_dev, inverse: bool):
    # mean and std_Dev are not used we are just lazy and dont want to change the code
    if inverse:
        data = data_input*(np.max(data_input)-np.min(data_input))+np.min(data_input)
    else:
       data =  (data_input-np.min(data_input))/(np.max(data_input)-np.min(data_input))
    return data
...
if __name__ == "__main__":
    ...
    #Converting data to linear scale
    S11_vals = db_to_linear(S11_vals, False)
    S11_parameterized = db_to_linear(S11_parameterized, False)
    combined_gain = db_to_linear(combined_gain, False)
    efficiency = db_to_linear(efficiency, False)
    ...
    if VERIFY_TEST_SET:
        ...
        test_indices = []
        for idx, i in enumerate(db_to_linear(normalize_data((y_test[:,:1001]),mean = 1, std_dev = 1, inverse=True),True)):
        ...
        for idx, i in enumerate(random_indices):
            ...
            plt.plot(frequency, db_to_linear(normalize_data(y_test[i][0:1001],np.mean(S11_vals),np.std(S11_vals),True),True), label='test')
            plt.legend()
            plt.grid(True)
            plt.ylim([-40,2])
        plt.show()
    ...
    if PLOT_TRAIN:
        ...
        # pred_vector = np.asarray([np.concatenate((y_pred_s11[i], [std_dev_pred[i]], [efficiency_pred[i]]))for i in range(y_pred_s11.shape[0])])

        # _, X_test, _, y_test = train_test_split(input_vector, pred_vector, test_size=0.3, shuffle=True, random_state=42)
        ...
        y_pred_s11 = db_to_linear(y_pred_s11, True)
        std_dev_pred = db_to_linear(std_dev_pred, True)
        efficiency_pred = db_to_linear(efficiency_pred, True)
    ...
    if PLOT_TEST: 
        ...
        for idx, i in enumerate(random_indices):
            plt.subplot(5, 2, idx+1)
            plt.plot(frequency, y_pred_s11[i], label=f'pred{i}')
            plt.plot(frequency, db_to_linear(normalize_data(y_test[i][:1001],np.mean(S11_vals),np.std(S11_vals),True),True), label=f'test{i}')
            plt.legend()
            plt.grid(True)
            plt.ylim([-40,2])
        #plt.show()
