...
# Makes the poles and zeroes into a plottable function
def rational_func(x, *args):
    global Number_of_poles
    eps = 1e-8
    def rational(data, p, q):
        return np.polyval(q, data) / np.polyval(p + [1.0], data)+eps
...
if __name__ == "__main__":
    ...
    print(f"s11_par_flat: {np.asarray(s11_parameterized_flat).shape}")
    ...
    print(f"S11_parameterized_flat_norm shape: {s11_parameterized_flat_norm.shape}")
   ...
   # print(f"s11_param_flat  = {s11_parameterized_flat[1]}")
    ...
    for layer in range(2, MAX_LAYERS):
        ...
        for j in range(50):
            ...
            if PLOT_TRAIN:
                ...
                plt.ylim([0, 1])
                plt.savefig(f'{path}data/DNN_results/sigmoid/train_loss/MADStrain_loss_layer{layer+1}/loss_{(j+1)*100}.png')
                plt.close()
            ...
            if PLOT_TEST:
                ...
                for idx, i in enumerate(random_indices):
                    ...
                    plt.grid(True)
                    plt.ylim([-40,2])
                plt.savefig(f'{path}data/DNN_results/sigmoid/test_pred/MADStest_pred_layer{layer+1}/test_pred_{(j+1)*400}.png')
                plt.close()
    ...
    plt.close()
    np.savetxt(f'{path}data/DNN_results/sigmoid/MADSrun_time.txt', run_time)


#     C:\Users\madsl\AppData\Local\Programs\Python\Python39\lib\site-packages\numpy\lib\polynomial.py:780: RuntimeWarning: overflow encountered in multiply
#   y = y * x + pv
# c:\Users\madsl\Dropbox\AAU\EIT 7. sem\P7\Python6_stuff\MachineLearning\MadsNeural_network.py:25: RuntimeWarning: invalid value encountered in divide
